...
\title[Заголовок]{Модели пространства состояний в задачах классификации сигналов ЭКоГ}
...
\begin{frame}{Постановка задачи классификации сигнала ЭКоГ}
	...
	$$ L(\bw) = -\dfrac{1}{M} \sum\limits_{m=1}^M \biggl[ y_m \log(f(\bw, \bx)) + (1 - y_m) \log(1 - f(\bw, \bx)) \biggr] + \lambda ||w||^2$$
	
	Оптимизационная задача: $\hat{\bw} = \underset{\bw}{\arg \min} L(\bw)$
\end{frame}
	...
	\begin{table}[bhtp]
		\begin{tabularx}{0.95\textwidth}{|X|X|}
			...
			Преимущества & Недостатки \\ \hline
				есть реализация во всех фреймворках глубокого обучения & не работают с данными, содержащими пропуски и/или разной частотой сэмплирования \\ относительно быстрое обучение и предсказание модели & легко переобучается \\ \hline
		\end{tabularx}
	\end{table}
\end{frame}
...
\begin{frame}{Свойства модели S4}
	...
	\begin{table}[bhtp]
		\begin{tabularx}{0.95\textwidth}{|X|X|}
			\hline
			Преимущества & Недостатки \\ \hline
			хорошо работает с данными, содержащими долговременные зависимости & не работают с данными, содержащими пропуски и/или разной частотой сэмплирования \\ 
			существует эффективная реализация & \\
			\hline
		\end{tabularx}
	\end{table}
